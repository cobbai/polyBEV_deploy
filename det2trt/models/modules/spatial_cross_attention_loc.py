from det2trt.models.utils import LINEAR_LAYERS, multi_scale_deformable_attn_pytorch
import warnings
import torch
...
from third_party.bev_mmdet3d import MultiScaleDeformableAttnFunction_fp32, \
...
@ATTENTION.register_module("SpatialCrossAttention_loc")
...
@ATTENTION.register_module("MSDeformableAttention3D_loc")
class MSDeformableAttention3D(BaseModule):
    ...
    def forward_trt(
        ...
        **kwargs,
        ...
        value = self.value_proj(value).view(2, -1, self.num_heads, self.embed_dims // self.num_heads)

        assert (spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum() == value.shape[1]

        sampling_offsets = self.sampling_offsets(query).view(
            2, -1, self.num_heads, self.num_levels, self.num_points, 2)
        attention_weights = self.attention_weights(query).view(
            2, -1, self.num_heads, self.num_levels * self.num_points)

        attention_weights = attention_weights.softmax(-1)

        attention_weights = attention_weights.view(2, -1,
                                                   self.num_heads,
                                                   self.num_levels,
                                                   self.num_points)
